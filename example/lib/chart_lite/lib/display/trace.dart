...
class ScatterTrace {
  ...
  /// A list with only one element means that the value applies to all
  /// elements of the trace.
  List<TextPosition>? textPosition;
  ...
  AlignmentPlotly? xPeriodAlignment;
  ...
  /// Only relevant when the axis `type` is "date". Sets the alignment of data
  /// points on the x axis.
  AlignmentPlotly? yPeriodAlignment;
  ...
  Map<String, dynamic> toJson() {
    return <String, dynamic>{
      ...
      if (textPosition != null)
        'textposition': textPosition!.length == 1
            ? textPosition!.first
            : textPosition,
      if (textTemplate != null)
        'texttemplate': textTemplate!.length == 1
            ? textTemplate!.first
            : textTemplate,
      if (hoverText != null)
      ...
      if (hoverTemplate != null)
        'hovertemplate': hoverTemplate!.length == 1
            ? hoverTemplate!.first
            : hoverTemplate,
  ...
  static bool isValidHoverInfo(String value) {
    const basicPlus = {'x', 'y', 'z', 'text', 'name'};
    const other = {'all', 'none', 'skip'};
    if (value.contains('+')) {
